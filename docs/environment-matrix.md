...
| Kora            | Test only                              | TEST                               | LIVE only after explicit release approval |
...
| Paid listings   | Kora test only                         | Kora test only                     | Enabled only after live-payment release |

Secrets belong in local ignored files or Cloudflare secret bindings. Public identifiers and feature flags may be ordinary environment variables. Staging and production must not share database branches, storage buckets, auth secrets or Kora keys.
