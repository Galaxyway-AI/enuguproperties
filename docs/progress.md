...
- Category-aware property details for houses, land, commercial property and developments, including typed public facts and additional catalogue filters for bathrooms and land size.
...
- Stage 2.5 business identity/contact configuration, test-mode payment lock, Turnstile action/replay validation, controlled feature flags, staging noindex headers, hostile-upload quarantine state, job-run monitoring and operational runbooks.

## Database

Seven ordered migrations cover the relational foundation, workflow RPCs, private storage, operations/video processing, immutable release controls, structured property details and controlled-beta operations. Idempotent seed configuration contains provisional plans, locations, permission mapping and verification/document types. There are no seeded real accounts, active legal agreements or fabricated verification badges.

## Validation

- TypeScript, lint and the Cloudflare-compatible production build complete successfully. The final automated suite contains 30 passing checks, including private-media token tamper and expiry coverage.
...
Complete Neon Data API and trusted-domain configuration, add test integration secrets, exercise external integrations and provide professionally approved seller/legal wording before production launch. Confirmed company and contact details are now applied centrally.
