...
-- Source: supabase/migrations/0016_restore_public_listing_access.sql
...
-- Source: supabase/migrations/0017_listing_availability.sql
...
begin
  ...
  select * into p from public.properties where id=p_id for update;
  if p.id is null or p.seller_id is distinct from app_auth.uid() then
...
-- Source: supabase/migrations/0018_advertising_promotions.sql
...
begin
  ...
  if not app_private.active_user() or p.seller_id is distinct from app_auth.uid() then
    raise exception 'Listing not found.';
  end if;
  select * into plan from public.listing_plans where id=p.plan_id and active;
  ...
  if normalized is not null then
    ...
    where x.automatic and x.active and x.starts_at<=now() and (x.ends_at is null or x.ends_at>now())
      ...
      and (x.restricted_user_id is null or x.restricted_user_id=app_auth.uid())
      and (not x.first_listing_only or not exists(select 1 from public.properties q where q.seller_id=app_auth.uid() and q.id<>p.id))
      ...
      and (select count(*) from public.promotion_redemptions r where r.promotion_id=x.id and r.user_id=app_auth.uid() and r.status in ('reserved','redeemed'))<x.per_user_limit
  ...
  if promo.id is not null then
    ...
    if promo.restricted_user_id is not null and promo.restricted_user_id<>app_auth.uid() then raise exception 'Promotion code is assigned to another account.'; end if;
    if promo.first_listing_only and exists(select 1 from public.properties q where q.seller_id=app_auth.uid() and q.id<>p.id) then raise exception 'Promotion code is only valid for your first property advert.'; end if;
    ...
    if (select count(*) from public.promotion_redemptions r where r.promotion_id=promo.id and r.user_id=app_auth.uid() and r.status in ('reserved','redeemed'))>=promo.per_user_limit then raise exception 'You have already used this promotion.'; end if;
...
begin
  ...
  if promo.id is not null then
    ...
    select count(*) into user_uses from public.promotion_redemptions r where r.promotion_id=promo.id and r.user_id=app_auth.uid() and r.status in ('reserved','redeemed');
    if not promo.active or promo.starts_at>now() or (promo.ends_at is not null and promo.ends_at<=now()) then raise exception 'Promotion code is inactive or expired.'; end if;
    if promo.plan_id is not null and promo.plan_id<>plan.id then raise exception 'Promotion code is not valid for this advertising plan.'; end if;
    if promo.restricted_user_id is not null and promo.restricted_user_id<>app_auth.uid() then raise exception 'Promotion code is assigned to another account.'; end if;
    if promo.first_listing_only and exists(select 1 from public.properties q where q.seller_id=app_auth.uid() and q.id<>p.id) then raise exception 'Promotion code is only valid for your first property advert.'; end if;
  ...
  values(app_auth.uid(),p.id,plan.id,to_jsonb(plan),plan.price_minor,discount,final_amount,promo.id,case when final_amount=0 then 'paid' else 'pending' end,case when final_amount=0 then now() end)
  returning * into o;
  if promo.id is not null then
    ...
    values(promo.id,o.id,app_auth.uid(),p.id,promo.code,plan.price_minor,discount,final_amount,case when final_amount=0 then 'redeemed' else 'reserved' end,case when final_amount=0 then now() end);
  end if;
  if final_amount=0 then
    perform app_private.notify(app_auth.uid(),'promotion_applied','Free advertising offer applied',promo.name||' has been applied to '||p.reference||'. Submit your listing for moderation when ready.');
...
begin
  ...
  if p_id is null then
    ...
    values(normalized,trim(p_data->>'name'),kind,value,plan,starts,ends,max_uses,per_user,coalesce((p_data->>'first_listing_only')::boolean,false),coalesce((p_data->>'automatic')::boolean,false),restricted,coalesce((p_data->>'active')::boolean,true),app_auth.uid()) returning id into result;
  else
...
-- Source: supabase/migrations/0019_featured_advertising.sql
...
begin
  select * into p from public.properties where id=p_property for update;
  if not app_private.active_user() or p.id is null or p.seller_id is distinct from app_auth.uid() then
    raise exception 'Listing not found.';
  end if;
