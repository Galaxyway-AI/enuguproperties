...
declare p public.properties; result uuid; loc uuid; safe_details jsonb; begin
 ...
 if p_id is null then
  ...
  values(auth.uid(),p_data->>'title',p_data->>'category',coalesce(p_data->>'listing_purpose','sale'),p_data->>'property_type',loc,coalesce(p_data->>'description',''),(p_data->>'price_minor')::bigint,coalesce((p_data->>'negotiable')::boolean,false),nullif(p_data->>'bedrooms','')::int,nullif(p_data->>'bathrooms','')::int,nullif(p_data->>'toilets','')::int,nullif(p_data->>'living_rooms','')::int,nullif(p_data->>'parking_spaces','')::int,(p_data->>'land_sqm')::numeric,nullif(p_data->>'building_sqm','')::numeric,coalesce(p_data->>'property_condition',''),coalesce(p_data->>'furnishing',''),safe_details,coalesce(p_data->>'title_type',''),array(select jsonb_array_elements_text(coalesce(p_data->'features','[]')))) returning id into result;
