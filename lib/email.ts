  ...
  async send(message) {
    ...
    const r = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        text: message.text,
        html: `<div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;color:#192c26"><h2 style="color:#165a42">enugu properties</h2><h1>${escape(message.subject)}</h1><p style="line-height:1.8;white-space:pre-wrap">${escape(message.text)}</p>${message.accountLink === false ? "" : `<p><a href="${escape(appUrl("/account/dashboard"))}">View your account</a></p>`}<hr><p style="font-size:12px">${business.brandName} · ${business.legalName}<br>Never pay for a property solely on the basis of an online listing.</p></div>`,
      }),
